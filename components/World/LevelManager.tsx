...
import { GameObject, ObjectType, LANE_WIDTH, SPAWN_DISTANCE, REMOVE_DISTANCE, GameStatus, TARGET_COLORS, Difficulty } from '../../types';
import { audio } from '../System/Audio';

// Geometry Constants
const OBSTACLE_HEIGHT = 1.6;
const OBSTACLE_GEOMETRY = new THREE.ConeGeometry(0.9, OBSTACLE_HEIGHT, 4); // Reduced segments
const OBSTACLE_GLOW_GEO = new THREE.ConeGeometry(0.9, OBSTACLE_HEIGHT, 4); // Reduced segments
const OBSTACLE_RING_GEO = new THREE.RingGeometry(0.6, 0.9, 4); // Reduced segments

const GEM_GEOMETRY = new THREE.IcosahedronGeometry(0.3, 0);

// Alien Geometries
const ALIEN_BODY_GEO = new THREE.CylinderGeometry(0.6, 0.3, 0.3, 6); // Reduced segments
const ALIEN_DOME_GEO = new THREE.SphereGeometry(0.4, 8, 8, 0, Math.PI * 2, 0, Math.PI/2); // Reduced segments
const ALIEN_EYE_GEO = new THREE.SphereGeometry(0.1, 4, 4); // Reduced segments

// Missile Geometries
const MISSILE_CORE_GEO = new THREE.CylinderGeometry(0.08, 0.08, 3.0, 4); // Reduced segments
const MISSILE_RING_GEO = new THREE.TorusGeometry(0.15, 0.02, 8, 16); // Reduced segments

// Drone Geometries
const DRONE_BODY_GEO = new THREE.BoxGeometry(0.8, 0.2, 0.8);
const DRONE_WING_GEO = new THREE.BoxGeometry(1.2, 0.05, 0.3);

// Mine Geometries
const MINE_CORE_GEO = new THREE.SphereGeometry(0.5, 8, 8); // Reduced segments
const MINE_SPIKE_GEO = new THREE.ConeGeometry(0.1, 0.4, 3); // Reduced segments

// Sweeper Geometries
const SWEEPER_BODY_GEO = new THREE.BoxGeometry(1.5, 0.4, 0.4);
const SWEEPER_BLADE_GEO = new THREE.BoxGeometry(0.1, 1.2, 0.2);

// Hunter Geometries
const HUNTER_BODY_GEO = new THREE.OctahedronGeometry(0.6, 0);
const HUNTER_EYE_GEO = new THREE.BoxGeometry(0.4, 0.1, 0.1);

// Shadow Geometries
const SHADOW_LETTER_GEO = new THREE.PlaneGeometry(2, 0.6);
const SHADOW_GEM_GEO = new THREE.CircleGeometry(0.6, 16); // Reduced segments
const SHADOW_ALIEN_GEO = new THREE.CircleGeometry(0.8, 16); // Reduced segments
const SHADOW_MISSILE_GEO = new THREE.PlaneGeometry(0.15, 3);
const SHADOW_DRONE_GEO = new THREE.CircleGeometry(0.7, 16); // Reduced segments
const SHADOW_MINE_GEO = new THREE.CircleGeometry(0.6, 16); // Reduced segments
...
const getLetterInterval = (level: number, difficulty: Difficulty) => {
    let base = BASE_LETTER_INTERVAL;
    // On Hard, letters are further apart, requiring more survival time
    if (difficulty === Difficulty.EASY) base *= 0.8;
    else if (difficulty === Difficulty.HARD) base *= 1.2;
    
    return base * Math.pow(1.5, Math.max(0, level - 1));
};
...
const ParticleSystem: React.FC = () => {
    ...
    const activeParticleCount = isMobile ? 300 : PARTICLE_COUNT;
    ...
    useEffect(() => {
        const handleExplosion = (e: CustomEvent) => {
            ...
            let spawned = 0;
            const burstAmount = isMobile ? 20 : 40; 
    ...
    }, [particles, isMobile, activeParticleCount]);
...
export const LevelManager: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isMenuReset || isRestart || isVictoryReset) {
        ...
        nextLetterDistance.current = getLetterInterval(1, difficulty);
        ...
        nextLetterDistance.current = distanceTraveled.current - SPAWN_DISTANCE + getLetterInterval(level, difficulty);
  ...
  useFrame((state, delta) => {
    ...
    // Update Music Intensity & Suspense
    let minProximity = 100;
    objectsRef.current.forEach(obj => {
        if (obj.active && (obj.type === ObjectType.OBSTACLE || obj.type === ObjectType.ALIEN || obj.type === ObjectType.DRONE || obj.type === ObjectType.MINE)) {
            const d = Math.abs(obj.position[2] - playerPos.z);
            if (obj.position[2] < playerPos.z && d < minProximity) {
                minProximity = d;
            }
        }
    });
    audio.updateMusic(speed, minProximity);
    ...
    for (const obj of currentObjects) {
        ...
        if (obj.type === ObjectType.DRONE && obj.active) {
            ...
            const range = maxLane * LANE_WIDTH;
            obj.position[0] = Math.sin(time * 2 + obj.id.charCodeAt(0)) * range;
        }

        // Sweeper Horizontal Movement (Linear)
        if (obj.type === ObjectType.SWEEPER && obj.active) {
            const time = state.clock.elapsedTime;
            const maxLane = Math.floor(laneCount / 2);
            const range = maxLane * LANE_WIDTH;
            // Linear sweep from -range to range
            const period = 3.0;
            const phase = (time + obj.id.charCodeAt(0)) % period;
            const t = phase / period;
            const direction = Math.floor((time + obj.id.charCodeAt(0)) / period) % 2 === 0 ? 1 : -1;
            
            if (direction > 0) {
                obj.position[0] = -range + (t * 2 * range);
            } else {
                obj.position[0] = range - (t * 2 * range);
            }
        }

        // Hunter Movement (Follows Player)
        if (obj.type === ObjectType.HUNTER && obj.active) {
            // Only follow if far enough away to be fair
            if (obj.position[2] < -10) {
                const targetX = playerPos.x;
                const dx = targetX - obj.position[0];
                const followSpeed = 2.0 * safeDelta;
                obj.position[0] += Math.sign(dx) * Math.min(Math.abs(dx), followSpeed);
            }
        }
        ...
        if (obj.active) {
            ...
            if (obj.type === ObjectType.SHOP_PORTAL) {
                ...
                if (dx < 0.9) { // Slightly increased horizontal forgiveness
                     ...
                     const isDamageSource = obj.type === ObjectType.OBSTACLE || obj.type === ObjectType.ALIEN || obj.type === ObjectType.MISSILE || obj.type === ObjectType.DRONE || obj.type === ObjectType.MINE || obj.type === ObjectType.SWEEPER || obj.type === ObjectType.HUNTER;
                     ...
                     if (isDamageSource) {
                         ...
                         if (obj.type === ObjectType.OBSTACLE) {
                             ...
                             objBottom = 0.5;
                             objTop = 1.5;
                         } else if (obj.type === ObjectType.HUNTER) {
                             objBottom = 0.5;
                             objTop = 1.5;
                         }
    ...
    if (furthestZ > -SPAWN_DISTANCE) {
         ...
         if (isLetterDue) {
             ...
             if (availableIndices.length > 0) {
                 ...
                 const color = getTargetColor(chosenIndex);
                 ...
                 nextLetterDistance.current += getLetterInterval(level, difficulty);
                 hasChanges = true;
             } else {
            ...
            // Difficulty affects obstacle probability
            let obstacleThreshold = 0.20; // Medium
            if (difficulty === Difficulty.EASY) obstacleThreshold = 0.40; // 60% obstacles
            else if (difficulty === Difficulty.HARD) obstacleThreshold = 0.10; // 90% obstacles

            const isObstacle = Math.random() > obstacleThreshold;

            if (isObstacle) {
                const pType = Math.random();
                
                // Decide enemy type based on level and probability
                if (level >= 3 && pType < 0.10) {
                    // HUNTER: Follows player
                    audio.playEnemySpawn();
                    const lane = getRandomLane(laneCount);
                    keptObjects.push({
                        id: uuidv4(),
                        type: ObjectType.HUNTER,
                        position: [lane * LANE_WIDTH, 1.2, spawnZ],
                        active: true,
                        color: '#ff00ff'
                    });
                } else if (level >= 2 && pType < 0.20) {
                    // SWEEPER: Moves across lanes
                    audio.playEnemySpawn();
                    const maxLane = Math.floor(laneCount / 2);
                    const startLane = Math.random() > 0.5 ? maxLane : -maxLane;
                    keptObjects.push({
                        id: uuidv4(),
                        type: ObjectType.SWEEPER,
                        position: [startLane * LANE_WIDTH, 1.0, spawnZ],
                        active: true,
                        color: '#ffff00'
                    });
                } else if (level >= 2 && pType < 0.30) {
                    // DRONE: Oscillating enemy
                    audio.playEnemySpawn();
                    const lane = getRandomLane(laneCount);
                    keptObjects.push({
                        id: uuidv4(),
                    ...
                    // MINE: Stationary pulsing hazard
                    audio.playEnemySpawn();
                    const lane = getRandomLane(laneCount);
                    keptObjects.push({
                        id: uuidv4(),
                ...
                } else if (level >= 2 && pType < 0.55) {
                    // ALIEN: Fires missiles
                    audio.playEnemySpawn();
...
const GameEntity: React.FC<{ data: GameObject }> = React.memo(({ data }) => {
    ...
    const shadowGeo = useMemo(() => {
        ...
        if (data.type === ObjectType.DRONE) return SHADOW_DRONE_GEO;
        ...
        <group ref={groupRef} position={[data.position[0], 0, data.position[2]]}>
            ...
            <group ref={visualRef} position={[0, data.position[1], 0]}>
                ...
                {data.type === ObjectType.DRONE && (
                    <group>
                        <mesh geometry={DRONE_BODY_GEO}>
                            <meshStandardMaterial color="#333333" metalness={0.9} roughness={0.1} />
                        </mesh>
                        <mesh position={[0, 0, 0.4]} geometry={DRONE_WING_GEO}>
                            <meshStandardMaterial color="#00ffff" emissive="#00ffff" emissiveIntensity={0.5} />
                        </mesh>
                        <mesh position={[0, 0, -0.4]} geometry={DRONE_WING_GEO}>
                            <meshStandardMaterial color="#00ffff" emissive="#00ffff" emissiveIntensity={0.5} />
                        </mesh>
                        <mesh position={[0.3, 0.1, 0]} geometry={ALIEN_EYE_GEO}>
                ...
                {data.type === ObjectType.MINE && (
                    <group>
                        <mesh geometry={MINE_CORE_GEO}>
                            <meshStandardMaterial color="#222222" metalness={0.8} roughness={0.2} />
                        </mesh>
                        {/* Spikes */}
                        {[...Array(6)].map((_, i) => (
                            <group key={i} rotation={[i * Math.PI / 3, i * Math.PI / 4, 0]}>
                                <mesh position={[0, 0.5, 0]} geometry={MINE_SPIKE_GEO}>
                ...
                {data.type === ObjectType.SWEEPER && (
                    <group>
                        <mesh geometry={SWEEPER_BODY_GEO}>
                            <meshStandardMaterial color="#444444" metalness={0.8} roughness={0.2} />
                        </mesh>
                        <mesh position={[0.6, 0, 0]} geometry={SWEEPER_BLADE_GEO}>
                ...
                {/* --- HUNTER --- */}
                {data.type === ObjectType.HUNTER && (
                    <group>
                        <mesh geometry={HUNTER_BODY_GEO}>
                            <meshStandardMaterial color="#111111" metalness={0.9} roughness={0.1} />
                        </mesh>
                        <mesh position={[0, 0, 0.4]} geometry={HUNTER_EYE_GEO}>
                            <meshBasicMaterial color="#ff00ff" />
                        </mesh>
                        <pointLight color="#ff00ff" intensity={1} distance={2} />
                    </group>
                )}
